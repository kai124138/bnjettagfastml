...
| **Resource-strategy ROM weights vs the ≤2-digit rule** | ❌→🛠 | **The Vitis ≤2-signed-digit DSP-free rule applies ONLY to compile-time constants.** With `Strategy: Resource` (the RF-folded deployable point), hls4ml stores weights in BRAM/ROM — runtime operands — and Vitis infers REAL multipliers: in-weight ±β̃ (an ~8-bit fixed value) synthesized to **256 DSPs** on a 256×256 "binary" dense at RF=256 (measured; the QKeras frontend avoids this only because it emits literal 1-bit `ap_uint<1>` weights with a dedicated `weight_binary` product class — the HGQ2/keras-v3 frontend has no such special-case). Fixes, in order of preference: (1) keep datapath weights **pure ±1** (2-bit operands → mux/negate, DSP-free even from ROM) and carry β̃ in a downstream affine; (2) `Strategy: Latency`, which inlines weights as constants so ±1 folds into add/sub trees (RF still bounds multiplier instances via `multiplier_limit`). |
